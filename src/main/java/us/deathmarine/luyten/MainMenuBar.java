...
public class MainMenuBar extends JMenuBar {
    ...
    public MainMenuBar(MainWindow mainWnd) {
        ...
        final JMenu versionMenu = new JMenu("Mappings");
        versionMenu.add(new JMenuItem("..."));
        ...
        new Thread() {
            public void run() {
                try {
                    ...
                    buildVersionMenu(versionMenu);
                    refreshMenuPopup(versionMenu);
    ...
    private void buildVersionMenu(JMenu versionMenu) {
        versionMenu.removeAll();
        versionGroup = new ButtonGroup();
        for (String version : ExtractMappings.versions) {
            JRadioButtonMenuItem item = new JRadioButtonMenuItem(version);
            item.addActionListener(e -> ExtractMappings.reloadMappings(version));
            versionGroup.add(item);
            versionMenu.add(item);
        }
        JRadioButtonMenuItem none = new JRadioButtonMenuItem("None");
        none.setEnabled(true);
        none.addActionListener(e -> ExtractMappings.reloadMappings(null));
        versionGroup.add(none);
        versionMenu.add(none);
